function [TCR,CR,OR,NON]= bbo_stats(numoftargets,napppos,w1,w2,w3,w4,plotnode)
    %this is BBO-DE algo.
...
function newGeneration= bbo_de(generation,nTargetPoints,sensingRange,communicationRange,bsLocation,targetpoints,bbopopulationsize,npopulationSize,w1,w2,w3,w4)
    ...
    for i = 1:bbopopulationsize
        ...
        jrand = rand();
        if jrand < mutationprobability && i > 5 % for 5 elite solution there is no mutation
            %mutate the solution
            trialvector= randi(500,npopulationSize,3);
