function [TCR,CR,OR,NON]= ga_stats(numoftargets,napppos,w1,w2,w3,w4,plotnode)
    ...
    nTargetPoints = numoftargets; % number of target points
    bsLocation = [250, 250]; % base station location

    % Define the GA parameters
    npopulationSize = napppos; % size of the population
    ...
    for i = 1:maxIteration
        ...
        for j = 1:GApopulationsize
            fitness(j,1) = evaluateFitness(generation(:,:,j), nTargetPoints, sensingRange, communicationRange, bsLocation,targetpoints,w1,w2,w3,w4);
        end
        ...
        % Perform mutation
        mutatedGeneration = mutation(offspringGeneration, mutationRate,fitness,GApopulationsize, nTargetPoints, sensingRange, communicationRange, bsLocation,targetpoints,w1,w2,w3,w4);
...
function mutatedGeneration = mutation(generation, mutationRate,fitness,GApopulationSize, nTargetPoints, sensingRange, communicationRange, bsLocation,targetpoints,w1,w2,w3,w4)
    ...
    for i = 1:GApopulationSize
        if rand()< mutationRate
            ...
            fitness(i,1) = evaluateFitness(mutantvector, nTargetPoints, sensingRange, communicationRange, bsLocation,targetpoints,w1,w2,w3,w4);
        end
    end
end
...
function fitness = evaluateFitness(positions, nTargetPoints, sensingRange, communicationRange, bsLocation,targetpoints,w1,w2,w3,w4)
